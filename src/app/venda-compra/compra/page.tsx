...
export default function CompraPage() {
  ...
  const [form, setForm] = useState({
    numero: '', apelido: '', dataCompra: new Date().toISOString().split('T')[0],
    sexo: 'F', pesoEstimado: '', origem: '', observacoes: '', corBrinco: '',
  });
    ...
    <FormPage titulo="Informar Compra">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Campo label="Nome/Apelido"><Input value={form.apelido} onChange={e => set('apelido', e.target.value)} /></Campo>
        </div>
        <Campo label="Informe a Data da Compra">
          <Input type="date" value={form.dataCompra} onChange={e => set('dataCompra', e.target.value)} required />
        </Campo>
        <div className="grid grid-cols-2 gap-3">
          <Campo label="Sexo">
          ...
          <Campo label="Peso Estimado (KG)">
            <Input type="number" step="0.1" value={form.pesoEstimado} onChange={e => set('pesoEstimado', e.target.value)} />
          </Campo>
        </div>
        <Campo label="Cor do Brinco">
          <CorBrinco value={form.corBrinco} onChange={v => set('corBrinco', v)} />
        </Campo>
        <Campo label="Informe a Origem do Animal">
          <Input value={form.origem} onChange={e => set('origem', e.target.value)} placeholder="Fazenda / Cidade de origem" required />
        </Campo>
